...
asset ids (Photo→Asset), scenarios (M4), unread badges (needs a `ThreadRead`
event + reducer semantics), a global toast surface (the `DeviceFrame` overlay
slot is the seam), per-theme motion tokens.
